...
const SingleVideoPage = ({ params, token }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const videoResponse = await api.get(`/Youtube/${id}`, {
        ...
        setVideoData(videoResponse.data.youtubeLink);
  ...
  useEffect(() => {
    if (videoData) {
      form.reset({
        ...
        Tags: videoData.tags?.find(tag => tag.name !== "featured")?.id || undefined,
        featured: videoData.tags?.some(tag => tag.name === "featured") || false,
      });
    }
  }, [videoData, form]);
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      await api.post(`/Youtube/update`, requestData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div>
      <ToastContainer />
      <p><strong>ID:</strong> {videoData?.id}</p>
      <p><strong>Created At:</strong> {formatDate(videoData?.createdAt)}</p>
      <p><strong>Updated At:</strong> {formatDate(videoData?.updatedAt)}</p>
      <Form {...form}>
...
export default withAuth(SingleVideoPage);
