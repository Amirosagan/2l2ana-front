  ...
  return (
    <div className="mt-12 px-4 sm:px-10 lg:mx-20 mb-5">
      ...
      <div className='mt-10 px-4 sm:px-10'>
        <div className="flex items-center cursor-pointer" onClick={toggleSection}>
          <h2 className='font-bold text-2xl tajawal-bold'>ملفي الشخصي</h2>
          {isSectionOpen ? <ChevronUp className='mt-1 mx-2'/> :   <ChevronDownIcon className='mt-1 mx-2 ' />}
        </div>
        <div className={`transition-all duration-300 ${isSectionOpen ? 'max-h-full opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
          {role === 'Doctor' ? (
