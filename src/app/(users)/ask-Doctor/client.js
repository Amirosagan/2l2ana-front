...
const AskDoctor = () => {
    ...
    const validate = () => {
        const newErrors = {};
        if (!title) newErrors.title = "عنوان السؤال مطلوب";
        if (!content) newErrors.content = "محتوى السؤال مطلوب";
        if (!selectedTag) newErrors.selectedTag = "اختر تصنيفًا للسؤال";
        setErrors(newErrors);
                ...
                <div className="w-full flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-shrink-0 w-full md:w-1/3">
                        <FormControl className="w-full" error={!!errors.selectedTag}>
                            <InputLabel id="tag-select-label" style={{ backgroundColor: 'white' }}>اختر تصنيفًا للسؤال</InputLabel>
                            <Select
    labelId="tag-select-label"
    value={selectedTag}
    onChange={(e) => setSelectedTag(e.target.value)}
    label="اختر تصنيفًا للسؤال"
    ...
    {tags
        ...
        .map(tag => (
            <MenuItem key={tag.id} value={tag.id}>{tag.name}</MenuItem>
                ...
                <p className="text-sm text-red-600"> بعد ارفاق السؤال سيتم عرضه للجمهور، لذا يرجي توخي الحذر وازالة اي معلومات خاصة </p>
                <button 
