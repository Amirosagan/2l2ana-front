...
const Header = () => {
  ...
  const [language, setLanguage] = useState(locale === 'ar' || locale === 'en' ? locale : 'en'); // Initialize with 'en' if locale is not 'ar' or 'en'
  const t = useTranslations('Header');

  // Use window.location.pathname to get the current URL path
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setCurrentPath(window.location.pathname);
      setActiveItem(window.location.pathname); // Set active item as the current path
    }
  }, []);
