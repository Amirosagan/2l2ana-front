...
const UpdateDoctorForm = ({ doctorId }) => {
  ...
  const handleCategoryChange = (newValue) => {
    setProfileData({ ...profileData, category: newValue ? newValue.value : '' });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className='border-2 rounded-md p-4 mt-5'>
      <ToastContainer />
      <div className="mb-4">
      ...
      <div className="mb-4">
        <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="category">
          Category <span className="text-red-500">اذا لم تجد تخصصك المطلوب قم بكتابته</span>
        </label>
        <CreatableSelect
          name="category"
          options={categories}
          ...
          value={categories.find(option => option.value === profileData.category) || { value: profileData.category, label: profileData.category }}
      ...
      <div className="mb-4">
        ...
        <Select
          ...
          classNamePrefix="select"
          value={weekDaysOptions.filter(option => profileData.weekDaysAvailableIds.includes(option.value))}
      ...
      <div className="mb-4">
        ...
        <Select
          ...
          className="basic-multi-select"
          classNamePrefix="select"
          value={dayTimesOptions.filter(option => profileData.dayTimesAvailableIds.includes(option.value))}
          onChange={handleSelectChange}
        />
      </div>
      <div className="mb-4">
