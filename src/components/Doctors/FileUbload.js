    ...
    return (
        <div className='border-2 rounded-md p-4 mt-5 text-center'>
            <h1 className='tajawal-bold text-xl sm:text-2xl mt-4 text-gray-400 flex items-center justify-center text-center flex-wrap'>
                احتفظ بكل <span className='tajawal-extrabold mx-2 text-primary'>بياناتك الصحية</span> في مكان واحد بكل سهولة وأمان
                <ClickAwayListener onClickAway={handleTooltipClose}>
                    <div className="mx-2">
                        <Tooltip
                            title={<Typography variant="body2" sx={{ p: 1 }}>بياناتك الطبية مؤمنة بالكامل. لا يمكن لأحد رؤيتها إلا عند الاشتراك مع أحد الأطباء، وهو الوحيد الذي يستطيع الاطلاع عليها.</Typography>}
                            open={open}
            ...
            <div {...getRootProps()} className="mt-7">
                <input {...getInputProps()} />
                <button className="bg-primary text-white px-4 py-2 rounded tajawal-bold hover:bg-primary-dark transition" disabled={loading}>
                    {loading ? "جاري تحميل الملفات ..." : "رفع ملفات (حتى 3 صور أو ملفات PDF)"}
                </button>
            </div>
            <div className="mt-4 flex flex-wrap gap-4 justify-center">
                {isFetching ? (
                    Array.from({ length: 3 }).map((_, index) => (
                        <div key={index} className="relative w-48 h-64 border rounded-md p-2 flex flex-col items-center justify-center">
                    ...
                    medicalFiles.map((file, index) => (
                        <div key={index} className="relative w-48 h-64 border rounded-md p-2 flex flex-col items-center justify-between">
                            ...
                            >
                                {downloadLoading[file.id] ? <CircularProgress size={24} color="inherit" /> : 'تحميل'}
                            </button>
                        </div>
                    ))
                )}
            ...
            {modalImage && (
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50" onClick={closeModal}>
                    <Image width={500} height={500} src={modalImage} alt="Full Size" className="max-w-full max-h-full" />
                </div>
            )}
        </div>
    );
